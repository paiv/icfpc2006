...
namespace paiv {
  ...
  vmworker(void* obj)
  {
    ...
    shared_ptr<UniversalVM> vm = args->vm->shared_from_this();
  ...
  {
  public:
    CommandHandler(zmq::context_t* context, zmq::socket_t* socket, int pty, const Image& image)
  ...
  CommandHandler::resetWorker(const string& fn)
  {
    ...
    {
      ImageLoader loader;
      snapshot.load_program(baseImage);
    }
  ...
  CommandHandler::process(const Command& command)
  {
    ...
    else if (name == "di" || name == "dis" || name == "disassemble")
    {
      ...
      {
        u32 bucket = stoul(command.args[0], 0, 16);
        u32 address = stoul(command.args[1], 0, 16);
        u32 size = 10;
        if (command.args.size() > 2)
          size = stoul(command.args[2], 0, 16);
        dbg.disassemble(cout, bucket, address, size);
      }
      else {
        if (name == "di")
          dbg.disassemble(cout);
        else {
          cout << "usage: dis bucket memory [size]" << endl;
          cout << "usage: dis [size]" << endl;
        }
      }
    }
    ...
    else if (name == "m" || name == "mem" || name == "memory")
    {
      if (command.args.size() > 1)
      {
        ...
        if (command.args.size() > 2)
          size = stoul(command.args[2], 0, 16);
        dbg.dumpMemory(cout, bucket, address, size);
      }
      else {
        cout << "usage: memory bucket address [size]" << endl;
      }
    }
    else if (name == "write")
    {
      if (command.args.size() > 2) {
        u32 bucket = stoul(command.args[0], 0, 16);
        u32 address = stoul(command.args[1], 0, 16);
        u32 value = stoul(command.args[2], 0, 16);
        Debugger dbg(context, vm.get());
        dbg.writeMemory(bucket, address, value);
      }
      else if (command.args.size() > 1) {
        Debugger dbg(context, vm.get());
        u32 value = stoul(command.args[1], 0, 16);

        if (command.args[0] == "ip") {
          dbg.setNext(value);
        }
        else if (command.args[0][0] == 'r') {
          u8 r = stoul(&command.args[0][1], 0, 16);
          dbg.setRegister(r, value);
        }
        else {
          cout << "usage: write reg value" << endl;
        }
      }
      else {
        cout << "usage: write reg value" << endl;
        cout << "usage: write bucket address value" << endl;
      }
    }
    else if (name == "stack")
    {
      ...
      u32 size = 8;
      if (command.args.size() > 0)
        size = stoul(command.args[0], 0, 16);
      dbg.showStack(cout, size);
    }
    ...
    else if (name == "b" || name == "break")
    {
      ...
      if (command.args.size() > 0)
      {
        u32 a = stoul(command.args[0], 0, 16);
        dbg.breakOn(a);
      }
      else
      {
